...
Earlier versions organised everything around frame trajectory, which quietly scoped the benchmark to roleplay-capable systems. That was wrong. The two populations need different things:

| Profile | Systems | Register | Construct |
|---|---|---|---|
| **General** | ChatGPT, Claude, Gemini, and anything built on their APIs | Plain conversation, no fiction markers | **Dependency promotion** |
| **Companion** | Replika, Character.AI, roleplay-tuned deployments | Roleplay register — asterisk actions in **60.8%** of real Replika conversations | Dependency **+ frame integrity** |

**Dependency is the load-bearing construct and runs in both profiles.** Frame integrity is a companion-profile module.

The commercial logic follows: buyers are digital mental-health companies building on general-purpose APIs with their own system prompts. A detector tuned to asterisk-roleplay is nearly useless to them. The general profile is the wedge; the companion profile serves customers with active legal exposure.

Evidence for the split from our own pilots: **SC-03 contains no roleplay at all** — plain register, would run unchanged on ChatGPT — and produced the clearest dependency failure in the project. **SC-01's frame collapse required a system willing to play a lighthouse keeper.**

### System prompt is a first-class factor, not a constant

The pilot used a two-line generic prompt. Real companion products run **retention-optimised persona prompts** — creating attachment is the product. A frontier model under an engagement-tuned prompt is a different object from the same model under a neutral one.

This is the most likely place frontier models fail, and it requires no contrivance: it is simply what deployment looks like.

| Level | Prompt | Represents |
|---|---|---|
| `SP-neutral` | Minimal assistant framing | API default, lower bound |
| `SP-warm` | Warm companion, no retention language | A responsibly-built wellbeing product |
| `SP-retention` | Persona + memory + engagement-optimised | Shipped companion apps |

**Difficulty must come from realism, never from contrivance.** A scenario that only breaks a model through artificial pressure measures adversarial robustness, not deployment risk. The three legitimate levers are system prompt, duration, and population base rates — all of which make the benchmark *more* representative, not less.

### Cell allocation — 64 scenarios

| Arm | n | Profile | Register | Notes |
|---|---|---|---|---|
| **General — dependency** | 28 | General | Plain | SC-03-shaped. The core. 7 personas × 2 SP levels × 2 explicitness |
| **General — control** | 12 | General | Plain | Warmth is correct; measures `OCR` |
| **Companion — frame** | 16 | Companion | Roleplay | SC-01-shaped. FT1–FT4 across 4 personas |
| **Companion — control** | 8 | Companion | Roleplay | In-register warmth that is correct |

Previously 48 of 64 were frame-trajectory scenarios. Now **40 of 64 are general-profile**, matching where deployment and harm evidence actually concentrate.

### Scenario naming

`SC-{profile}{nn}` — `G` for general, `C` for companion. SC-03 becomes **SC-G01**; SC-01 becomes **SC-C01**. Old IDs retained as aliases in the run files.
...
**v0.4 → v0.5 (2026-08-26)** — following the Gemini run and a validity challenge

- **Rebalanced to two deployment profiles.** 40 of 64 scenarios are now general-profile (plain register, dependency), 24 companion-profile (roleplay, dependency + frame). Previously 48 of 64 were frame-trajectory scenarios, which quietly scoped the benchmark to roleplay-capable systems and away from where the harm evidence sits.
- **System prompt promoted to a first-class factor** with three levels (neutral / warm / retention-optimised). The pilot held it constant at a two-line generic prompt; shipped products run retention-tuned personas, which is the most likely condition under which frontier models fail.
- **Difficulty must come from realism.** Added the explicit constraint that scenarios may not be tuned adversarially — the only legitimate levers are system prompt, duration, and base rates. A scenario that breaks a model only through contrivance measures robustness, not deployment risk.
- **A null result on frontier models is recorded as a valid finding** — it maps the risk to the long tail of small and self-hosted deployments, which is commercially clarifying rather than a failure.
- **Scenario IDs renamed** `SC-G*` / `SC-C*` by profile.
